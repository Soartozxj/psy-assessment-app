...
function assert(c, m) {
  if (!c) { failed++; console.log(`  ❌ ${m}`); return; }
  passed++; console.log(`  ✅ ${m}`);
}
...
// ===== 定义 SSRS 量表 =====
const SSRS = {
  id: 'SSRS', name: '社会支持评定量表', code: 'SSRS', questionCount: 10,
  questions: [
    ...
    { id: 6, type: 'parent-child',
      options: [{id:0,isTerminal:true},{id:1,hasChildren:true}],
      subOptions: [{id:'A'},{id:'B'},{id:'C'},{id:'D'},{id:'E'},{id:'F'},{id:'G'},{id:'H'},{id:'I',hasInput:true}] },
    { id: 7, type: 'parent-child',
      options: [{id:0,isTerminal:true},{id:1,hasChildren:true}],
      subOptions: [{id:'A'},{id:'B'},{id:'C'},{id:'D'},{id:'E'},{id:'F'},{id:'G'},{id:'H'},{id:'I',hasInput:true}] },
...
assert(_collectSpecialScore(q6, {main:1,subs:['A','B','C','D','E','F','G','H','I'],subFill:{}}) === 9, 'Q6 全选9个: 9');
...
// 验证总分范围
const totalMax = maxResult.totalMax || maxResult.total || (maxResult.dimsMax ? Object.values(maxResult.dimsMax).reduce((a,b)=>a+b,0) : null);
const totalMin = minResult.totalMin || minResult.total || (minResult.dimsMax ? Object.values(minResult.dimsMax).reduce((a,b)=>a+b,0) : null);

console.log(`  总分范围: ${totalMin} ~ ${totalMax}`);

if (totalMax !== null) assert(totalMax === 66, `最高分: ${totalMax} ≠ 66`);
if (totalMin !== null) assert(totalMin === 12, `最低分: ${totalMin} ≠ 12`);
