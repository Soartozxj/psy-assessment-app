...
async function closeModals(page) {
  await page.evaluate(() => {
    document.querySelectorAll('.privacy-modal, .privacy-modal-overlay, .fb-modal, .fb-modal-overlay, .delete-confirm-modal, .delete-confirm-overlay').forEach(el => {
...
(async () => {
  ...
  async function mockWxNavigateBack(page) {
    await page.evaluate(() => {
      ...
      window.wx = {
        miniProgram: {
          navigateBack: () => { window.__wxExitCalled = true; }
        }
      };
    });
  }
  ...
  try {
    ...
    await test('答题中途 _doWebViewBack 会被 confirm 阻断', async () => {
      ...
      // 模拟有答题
      await page.evaluate(() => { answers = { 1: 0, 2: 1 }; });
