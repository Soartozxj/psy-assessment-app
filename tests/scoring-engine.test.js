...
test('screening 所有条件不满足时为阴性', () => {
  const scale = {
    ...
    scoring: {
      ...
      screening: {
        logic: 'OR',
        conditions: [{ metric: 'totalScore', op: '>=', value: 10 }],
        positiveLabel: '阳性',
        negativeLabel: '阴性'
      }
    }
...
test('维度 AVG 计算', () => {
  const scale = {
    questions: makeQuestionsRange(4, 1, 5),
    scoring: {
      dimensions: [{ key: 'avg_dim', label: '均分维度', formula: 'AVG', items: '1-4' }],
      metrics: []
    }
  };
...
test('维度 transform 线性变换', () => {
  const scale = {
    questions: makeQuestionsRange(4, 0, 4),
    scoring: {
      dimensions: [{ key: 't_dim', label: '变换维度', formula: 'AVG', items: '1-4', transform: '1.25*x' }],
      metrics: []
    }
  };
  // 全选 index=3 → score=3 → avg=3 → 3*1.25=3.75
...
test('题目兼容无 id（自动编号）', () => {
  const scale = {
    questions: [{ options: [{ score: 5 }] }, { options: [{ score: 3 }] }],
    scoring: {
      metrics: [{ key: 'totalScore', formula: 'SUM', items: 'ALL' }]
    }
  };
...
test('screening 条件解析（operator → op）', () => {
  const ai = {
    screening: {
      logic: 'OR',
      conditions: [{ metric: 'total_score', operator: '>=', value: 10, label: '需评估' }],
      positiveLabel: '阳性',
      negativeLabel: '阴性'
    }
  };
...
test('PPCRS 中等分场景', () => {
  const ppcrs = {
    ...
    scoring: {
      ...
      metrics: [{ key: 'coparenting_total', label: '共同教养总分', formula: 'AVG', items: 'ALL' }],
      interpretation: [
...
test('grouped 基本计分：2组结合', () => {
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  // g_time=C=2, g_impact=B=2 → total=4
  const r = ScoringEngine.calculate(scale, { 1: { g_time: 'C', g_impact: 'B' } });
  assert.strictEqual(r.metrics.totalScore, 4);
});

test('grouped 最大分推导', () => {
  const q = makeGrouped([
    { id: 'g1', options: [{ score: 1 }, { score: 3 }] },
    { id: 'g2', options: [{ score: 0 }, { score: 5 }] }
  ]);
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  const r = ScoringEngine.calculate(scale, { 1: { g1: 'B', g2: 'A' } });
  assert.strictEqual(r.maxScores.total, 8); // 3+5
});

test('grouped 未答分组不计分', () => {
  const q = makeGrouped([
    {
      id: 'g_time',
      options: [
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  // only g_time answered
  const r = ScoringEngine.calculate(scale, { 1: { g_time: 'B' } });
  assert.strictEqual(r.metrics.totalScore, 5);
});

test('grouped LES 风格 4 组维度', () => {
  const q = makeGrouped([
    {
      id: 'g_time',
      options: [
        { id: 'A', label: '未发生', score: 0 },
        { id: 'B', label: '1年前', score: 1 },
        { id: 'C', label: '1年内', score: 2 },
        { id: 'D', label: '长期性', score: 3 }
      ]
    },
    {
      id: 'g_nature',
      options: [
        { id: 'A', label: '好事', score: 0 },
        { id: 'B', label: '坏事', score: 0 }
      ]
    },
    {
      id: 'g_impact',
    ...
    },
    {
      id: 'g_duration',
      options: [
        { id: 'A', label: '3月内', score: 1 },
        { id: 'B', label: '半年内', score: 2 },
        { id: 'C', label: '1年内', score: 3 },
        { id: 'D', label: '1年以上', score: 4 }
      ]
    }
  ]);
  // g_time=C=2, g_nature=B=0, g_impact=D=3, g_duration=C=3 → sum=8
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  const r = ScoringEngine.calculate(scale, { 1: { g_time: 'C', g_nature: 'B', g_impact: 'D', g_duration: 'C' } });
  assert.strictEqual(r.metrics.totalScore, 8);
  assert.strictEqual(r.maxScores.total, 11); // max per group: g_time=3, g_nature=0, g_impact=4, g_duration=4
});

test('grouped 多道题合计', () => {
  const q1 = makeGrouped([
    {
      id: 'g_time',
      options: [
        { id: 'A', score: 0 },
  ...
  q2.id = 2;
  const scale = { questions: [q1, q2], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1,2' }] } };
  // q1: g_time=B=2, g_impact=B=3 → 5, q2: g_time=A=0, g_impact=A=0 → 0 → total=5
  const r = ScoringEngine.calculate(scale, { 1: { g_time: 'B', g_impact: 'B' }, 2: { g_time: 'A', g_impact: 'A' } });
  assert.strictEqual(r.metrics.totalScore, 5);
  assert.strictEqual(r.maxScores.total, 10); // (2+3)×2
});

// ============================================================
// grouped multiply 乘法模式测试
// ============================================================
test('grouped multiply LES 风格：影响程度×持续时间×发生次数', () => {
  const q = {
    id: 1,
    type: 'grouped',
    formula: 'multiply',
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  // 发生次数=2(1年内) × 影响=3(重度) × 持续=4(1年以上) = 24
  const r = ScoringEngine.calculate(scale, { 1: { g_occurrence: 'C', g_impact: 'D', g_duration: 'D' } });
  assert.strictEqual(r.metrics.totalScore, 24);
  assert.strictEqual(r.maxScores.total, 48); // 3×4×4
});

test('grouped multiply 最大值推导', () => {
  const q = {
    id: 1,
    type: 'grouped',
    formula: 'multiply',
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  const r = ScoringEngine.calculate(scale, { 1: { g1: 'B', g2: 'B' } });
  assert.strictEqual(r.metrics.totalScore, 15); // 3×5
  assert.strictEqual(r.maxScores.total, 15);
});

test('grouped multiply 最小值推导（含0分选项→整体为0）', () => {
  const q = {
    id: 1,
    type: 'grouped',
    formula: 'multiply',
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  // min = 0 × 1 = 0
  const r = ScoringEngine.calculate(scale, { 1: { g1: 'A', g2: 'A' } });
  assert.strictEqual(r.metrics.totalScore, 0); // 0×1
});

test('grouped multiply 纯求和模式不受影响（无formula字段）', () => {
  const q = {
    id: 1,
    type: 'grouped', // 注意：没有 formula 字段
    groups: [
      {
        id: 'g1',
        options: [
  ...
  const scale = { questions: [q], scoring: { metrics: [{ key: 'totalScore', formula: 'SUM', items: '1' }] } };
  const r = ScoringEngine.calculate(scale, { 1: { g1: 'B', g2: 'B' } });
  assert.strictEqual(r.metrics.totalScore, 12); // 5+7 = 12，不是 5×7=35
  assert.strictEqual(r.maxScores.total, 12); // 求和最大值
});

// ============================================================
// grouped 条件过滤测试（按组内选项值筛选）
// ============================================================
test('grouped 条件过滤：只计好事事件', () => {
  const q1 = {
    id: 1,
    type: 'grouped',
    groups: [
      ...
      dimensions: [
        {
          ...
          formula: 'SUM',
          items: '1-2',
          condition: { group: 'g_nature', eq: 'A' }
        }
      ],
      metrics: []
    }
  };
  // q1: good(A), score=5 → included. q2: bad(B), score=10 → filtered out
  const r = ScoringEngine.calculate(scale, {
    1: { g_nature: 'A', g_score: 'A' },
    2: { g_nature: 'B', g_score: 'B' }
  });
  assert.strictEqual(r.dimensions[0].score, 6); // only q1: 1+5=6
});

test('grouped 条件过滤：只计坏事事件', () => {
  const q1 = {
    id: 1,
    type: 'grouped',
    groups: [
  ...
  assert.strictEqual(r.dimensions[0].score, 12); // only q2: 2+10=12
});

test('grouped 条件过滤：全局指标（不加条件=全部计入）', () => {
  const q1 = {
    id: 1,
    type: 'grouped',
    groups: [
  ...
  const scale = {
    ...
    scoring: {
      dimensions: [
        { key: 'positive', label: '正性', formula: 'SUM', items: '1-2', condition: { group: 'g_nature', eq: 'A' } },
        { key: 'negative', label: '负性', formula: 'SUM', items: '1-2', condition: { group: 'g_nature', eq: 'B' } }
      ],
      metrics: [
        { key: 'total', label: '总计', formula: 'SUM', items: '1-2' } // 无 condition = 全部
      ]
    }
  };
  const r = ScoringEngine.calculate(scale, {
  ...
  assert.strictEqual(r.dimensions[0].score, 6); // positive
  assert.strictEqual(r.dimensions[1].score, 12); // negative
  assert.strictEqual(r.metrics.total, 18); // total = 6 + 12
});
