...
function log(msg, level = 'info') {
  const colors = { info: '\x1b[36m', pass: '\x1b[32m', fail: '\x1b[31m', skip: '\x1b[33m', bold: '\x1b[1m', reset: '\x1b[0m' };
...
function recordResult(module, name, status, detail = '') {
  ...
  log(`${icon} [${module}] ${name}${detail ? ' \u2014 ' + detail : ''}`, status === 'pass' ? 'pass' : status === 'fail' ? 'fail' : 'skip');
}
...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      recordResult('\u9875\u9762\u52a0\u8f7d', '1.2 \u624b\u673a\u5916\u58f3+TabBar', shell && tabBar ? 'pass' : 'fail',
    ...
    try {
      ...
      recordResult('\u9875\u9762\u52a0\u8f7d', '1.3 \u91cf\u8868\u5217\u8868\u5c55\u793a', count >= 6 ? 'pass' : 'fail',
        `\u68c0\u6d4b\u5230 ${count} \u4e2a\u91cf\u8868\u5361\u7247`);
    } catch (e) {
    ...
    try {
      ...
      recordResult('\u9875\u9762\u52a0\u8f7d', '1.4 JS\u6587\u4ef6\u65e0404', jsErrors.length === 0 ? 'pass' : 'fail',
        jsErrors.length > 0 ? jsErrors.length + ' \u4e2a\u8d44\u6e90\u9519\u8bef' : '\u6240\u6709JS\u6b63\u5e38\u52a0\u8f7d');
    } catch (e) {
    ...
    try {
      ...
      recordResult('\u9996\u9875\u5bfc\u822a', '2.1 TabBar 4\u4e2aTab', count === 4 ? 'pass' : 'fail',
        `\u68c0\u6d4b\u5230 ${count} \u4e2aTab`);
    } catch (e) {
    ...
    try {
      ...
      recordResult('\u9996\u9875\u5bfc\u822a', '2.2 \u5206\u7c7b\u5bfc\u822a\u5c55\u793a', totalCats > 0 ? 'pass' : 'fail',
    ...
    try {
      ...
      if (scaleId) {
        await page.evaluate((id) => { gotoDetail(id); }, scaleId);
        await sleep(500);
        recordResult('\u9996\u9875\u5bfc\u822a', '2.3 \u70b9\u51fb\u91cf\u8868\u8fdb\u8be6\u60c5', 'pass',
          firstScaleName ? '\u8fdb\u5165\u300c' + firstScaleName + '\u300d' : '');
      } else {
        recordResult('\u9996\u9875\u5bfc\u822a', '2.3 \u70b9\u51fb\u91cf\u8868\u8fdb\u8be6\u60c5', 'fail', '\u672a\u83b7\u53d6\u5230\u91cf\u8868ID');
      }
    } catch (e) {
    ...
    try {
      ...
      recordResult('NPC\u6d4b\u8bc4', '3.1 \u6c89\u6d78\u6a21\u5f0f\u9ed8\u8ba4\u9009\u4e2d', isActive ? 'pass' : 'fail');
    } catch (e) {
    ...
    try {
      const npcResult = await page.evaluate(async () => {
        ...
        if (currentPage !== 'scale-detail' || !document.querySelector('#page-scale-detail.active')) {
          ...
          if (card) {
            ...
            if (m) { gotoDetail(parseInt(m[1])); await new Promise(r => setTimeout(r, 500)); }
          }
        }
        // 3.2 开始测评
        startAssessment();
        ...
        if (!confirmBtn || confirmBtn.disabled) return { ok: false, step: 'confirm-disabled', welcomeLen: welcomeText.length };
        confirmBtn.click();
        // 等待第一题选项就绪
        for (var j = 0; j < 50; j++) {
          if (typeof npcTyping !== 'undefined' && npcTyping) {
        ...
        function selectOption() {
          ...
          for (var k = 0; k < its.length; k++) {
            if (its[k].style.pointerEvents !== 'none') { its[k].click(); return true; }
          }
          its[0].click();
          return true;
        }
      ...
      recordResult('NPC\u6d4b\u8bc4', '3.3 \u6b22\u8fce\u8bed\u663e\u793a', npcResult.welcomeLen > 10 ? 'pass' : 'fail',
        npcResult.welcomeLen > 10 ? npcResult.welcomeLen + ' \u5b57' : '\u7a7a');
      recordResult('NPC\u6d4b\u8bc4', '3.4 \u786e\u8ba4\u5f00\u59cb', npcResult.step !== 'confirm-disabled' ? 'pass' : 'fail');
      recordResult('NPC\u6d4b\u8bc4', '3.5 \u81ea\u52a8\u7b54\u9898\u5b8c\u6210', npcResult.answered > 0 ? 'pass' : 'fail',
    ...
    try {
      ...
      recordResult('NPC\u6d4b\u8bc4', '3.6 \u7ed3\u679c\u9875\u5c55\u793a', isResult ? 'pass' : 'fail',
    ...
    try {
      ...
      recordResult('NPC\u6d4b\u8bc4', '3.7 AI\u8bca\u65ad\u5165\u53e3', aiBtn ? 'pass' : 'fail',
    ...
    try {
      const retryResult = await page.evaluate(async () => {
        ...
        while (MAX_ITER-- > 0) {
          if (isRes()) break;
          var ct = getCT();
          if (ct === '\u63d0\u4ea4\u6d4b\u8bc4') { clickCfm(); count++; await wait(1000); break; }
          if (selOpt()) {
            await wait(150);
            if (!clickCfm()) { await waitForOpts(); clickCfm(); }
            count++;
            await wait(250);
          } else { await wait(400); }
        }
      ...
      recordResult('\u4e8c\u6b21\u6d4b\u8bc4', '4.3 \u4e8c\u6b21\u6d4b\u8bc4\u53ef\u542f\u52a8', retryResult.textLen > 5 ? 'pass' : 'fail');
      recordResult('\u4e8c\u6b21\u6d4b\u8bc4', '4.4 \u4e8c\u6b21\u7b54\u9898\u5b8c\u6210', retryResult.answered > 0 ? 'pass' : 'fail',
    ...
    try {
      ...
      recordResult('\u4e8c\u6b21\u6d4b\u8bc4', '4.5 \u4e8c\u6b21\u6d4b\u8bc4\u540e\u8fd4\u56de\u9996\u9875', isHome ? 'pass' : 'fail');
    } catch (e) {
      recordResult('\u4e8c\u6b21\u6d4b\u8bc4', '4.5 \u4e8c\u6b21\u6d4b\u8bc4\u540e\u8fd4\u56de\u9996\u9875', 'fail', e.message);
    }
    ...
    try {
      ...
      recordResult('\u6d4b\u8bc4\u8bb0\u5f55', '5.2 \u8bb0\u5f55\u5217\u8868\u5c55\u793a', count >= 2 ? 'pass' : 'skip',
        count + ' \u6761\u8bb0\u5f55');
    ...
    try {
      const normalResult = await page.evaluate(async () => {
        ...
        while (MAX_ITER2-- > 0) {
          ...
          if (selOpt()) {
            await wait(150);
            if (!clickCfm()) { await new Promise(function(r) { setTimeout(r, 200); }); clickCfm(); }
            count++;
            await wait(250);
          } else { await wait(400); }
        }
      ...
      recordResult('\u666e\u901a\u6a21\u5f0f', '6.3 \u5f00\u59cb\u666e\u901a\u6a21\u5f0f\u6d4b\u8bc4', normalResult.ok ? 'pass' : 'fail');
      recordResult('\u666e\u901a\u6a21\u5f0f', '6.4 \u666e\u901a\u6a21\u5f0f\u7b54\u9898\u5b8c\u6210', normalResult.answered > 0 ? 'pass' : 'fail',
    ...
    try {
      ...
      recordResult('\u5176\u4ed6\u529f\u80fd', '7.1 \u65b0\u624b\u5f15\u5bfc\u5df2\u5b8c\u6210', guideDone ? 'pass' : 'skip',
        guideDone ? 'localStorage \u5df2\u6807\u8bb0' : '\u672a\u6807\u8bb0\uff08\u9996\u6b21\u4f7f\u7528\u4f1a\u5f39\u51fa\uff09');
    } catch (e) {
    ...
    try {
      ...
      if (scaleInfo) {
        recordResult('\u5176\u4ed6\u529f\u80fd', '7.2 SCALES\u6570\u636e\u5b8c\u6574', 'pass',
          `\u603b\u8ba1${scaleInfo.total}\u4e2a, \u8ba1\u5206${scaleInfo.withScoring}\u4e2a, AI\u8bca\u65ad${scaleInfo.withAiDiag}\u4e2a`);
      } else {
        recordResult('\u5176\u4ed6\u529f\u80fd', '7.2 SCALES\u6570\u636e\u5b8c\u6574', 'fail', 'SCALES \u672a\u5b9a\u4e49');
      }
    } catch (e) {
  ...
  console.log('');
  console.log('  ' + padStr('\u6a21\u5757', 20) + ' ' + padStr('\u901a\u8fc7', 6) + ' ' + padStr('\u5931\u8d25', 6) + ' ' + padStr('\u8df3\u8fc7', 6));
  console.log('  ' + '-'.repeat(40));
  for (const [mod, counts] of Object.entries(modules)) {
    console.log('  ' + padStr(mod, 20) + ' ' + ('\u2705 ' + counts.pass).padStart(5) + ' ' + ('\u274c ' + counts.fail).padStart(5) + ' ' + ('\u23ed\ufe0f ' + counts.skip).padStart(5));
  }
  console.log('  ' + '-'.repeat(40));
  console.log('  ' + padStr('\u603b\u8ba1', 20) + ' ' + ('\u2705 ' + results.passed).padStart(5) + ' ' + ('\u274c ' + results.failed).padStart(5) + ' ' + ('\u23ed\ufe0f ' + results.skipped).padStart(5));
  console.log('');

  if (results.errors.length > 0) {
    log('\u26a0\ufe0f \u8d2f\u7a7f\u5168\u8fc7\u7a0b\u7684 JS \u9519\u8bef (' + results.errors.length + ' \u4e2a):', 'fail');
