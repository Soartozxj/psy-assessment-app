...
(function () {
  ...
  var ScaleWizard = {
    ...
    renderStepNav: function () {
      var nav = document.getElementById('scale-wizard-nav');
      if (!nav) return;

      nav.innerHTML = this.steps
        .map(function (step) {
          var isActive = step.id === ScaleWizard.currentStep;
          var isCompleted = step.id < ScaleWizard.currentStep;
          var statusClass = isActive ? 'active' : isCompleted ? 'completed' : '';
    ...
    goToStep: function (stepNum) {
      ...
      // 滚动到顶部
      var modalBody = document.querySelector('#scale-modal .modal-body');
      if (modalBody) modalBody.scrollTop = 0;
    },

    // 显示指定步骤内容
    showStep: function (stepNum) {
      var steps = ['step-basic', 'step-questions', 'step-npc', 'step-preview'];
      var i;
    ...
    renderPreview: function () {
      var preview = document.getElementById('scale-preview-content');
      if (!preview) return;

      // 基本信息预览
      var name = document.getElementById('f-name').value || '-';
      var shortName = document.getElementById('f-shortname').value || name;
      var code = document.getElementById('f-code').value || '-';
      var emoji = document.getElementById('f-emoji').value || '📋';
      var color = document.getElementById('f-color').value || '#4A90D9';
      var desc = document.getElementById('f-desc').value || '暂无描述';
      var category = document.getElementById('f-category');
      var categoryName = category ? category.options[category.selectedIndex].text : '-';

      // 题目统计
      var qCount = typeof currentQuestions !== 'undefined' ? currentQuestions.length : 0;
      var preqCount = typeof currentPreQuestions !== 'undefined' ? currentPreQuestions.length : 0;

      // NPC 配置预览
      var counselorEl = document.getElementById('edit-counselor-info');
      var counselorName = counselorEl ? counselorEl.querySelector('.edit-select-name').textContent : '全局默认';

      var bgEl = document.getElementById('edit-bg-info');
      var bgName = bgEl ? bgEl.querySelector('.edit-select-name').textContent : '全局默认';
  ...
  // 向导样式（内联）
  // ====================================================
  var wizardStyles = document.createElement('style');
  ...
  var _originalCloseModal = window.closeModal;
