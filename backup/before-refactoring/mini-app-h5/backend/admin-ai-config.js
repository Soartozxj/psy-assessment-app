...
(function (global) {
  ...
  function renderProviderSelector() {
    var container = document.getElementById('ai-provider-selector');
    if (!container) return;
  ...
  function renderConfigForm() {
    var container = document.getElementById('ai-config-form');
    if (!container) return;

    var isDashscope = currentConfig.provider === 'dashscope';
    var isDeepseek = currentConfig.provider === 'deepseek';
  ...
  function renderStatusPanel(status) {
    var container = document.getElementById('ai-status-panel');
    if (!container) return;

    var statusClass = status.ok ? 'success' : 'error';
  ...
  AIConfig.onModelChange = function (model) {
    var provider = currentConfig.provider;
  ...
  AIConfig.loadFromServer = async function () {
    try {
      var statusEl = document.getElementById('ai-load-status');
      ...
      if (data.code === 0 && data.data) {
        ...
        if (statusEl) statusEl.textContent = '⚠️ 服务端无配置';
        if (typeof showToast === 'function') {
  ...
  AIConfig.checkServerStatus = async function () {
    var statusEl = document.getElementById('ai-status-result');
    if (statusEl) {
      statusEl.innerHTML = '<span class="ai-loading">🔄 检查中...</span>';
    }

    try {
      var res = await fetch('/api/ai-config/status');
      var data = await res.json();

      if (data.code === 0) {
        renderStatusPanel(data.data);
      } else {
        if (statusEl) statusEl.innerHTML = '<span class="ai-error">❌ 检查失败</span>';
      }
    } catch (e) {
      console.error('Failed to check status:', e);
      if (statusEl) statusEl.innerHTML = '<span class="ai-error">❌ 网络错误</span>';
    }
  };
  ...
  AIConfig.testConnection = async function () {
    var statusEl = document.getElementById('ai-test-result');
    if (statusEl) {
      statusEl.innerHTML = '<span class="ai-loading">🔄 测试中...</span>';
    }

    try {
      var res = await fetch('/api/ai-config/test', { method: 'POST' });
      var data = await res.json();
