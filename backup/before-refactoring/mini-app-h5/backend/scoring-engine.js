...
var ScoringEngine = (() => {
  ...
  function calcGroupedScore(q, scores, mode) {
    ...
    for (var _gi = 0, _grps = _toGroupArray(q.groups); _gi < _grps.length; _gi++) {
      var group = _grps[_gi];
  ...
  function _collectSpecialScore(q, ans, dimKey) {
    ...
    if (q.type === 'grouped') {
      ...
      var grps = q.groups || {};
      var grpArray = Array.isArray(grps)
        ? grps
        ...
        for (var ai = 0; ai < ans.length; ai++) {
          ...
          for (var gi = 0; gi < grpArray.length; gi++) {
            ...
            if (oid !== undefined && oid !== null) {
              ...
              if (opt) {
                if (_isMultiply) itemScore *= opt.score || 0;
                else itemScore += opt.score || 0;
              }
            }
          }
          total += itemScore;
      ...
      for (var gi = 0; gi < grpArray.length; gi++) {
        var grp = grpArray[gi],
          oid = ans[grp.id];
        // 如果指定了目标分组，跳过不匹配的分组
        if (targetGroup && grp.id !== targetGroup) continue;
        if (oid !== undefined && oid !== null) {
          var opt = (grp.options || []).find(function (o) {
            return o.id === oid;
          });
          ...
          if (opt) {
            if (_isMultiply) total *= opt.score || 0;
