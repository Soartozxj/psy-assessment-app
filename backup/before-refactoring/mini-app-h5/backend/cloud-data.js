...
(function () {
  ...
  function detectCloudEnv() {
    ...
    if (params.get('env') === 'cloud') {
      return params.get('envId') || 'default';
    }
  ...
  // 辅助：本地存储操作
  // ====================================================
  const STORAGE_KEY = 'psy_assessment_history';
  ...
  function deleteLocalHistory(id) {
    try {
      const arr = getLocalHistory().filter(function (h) {
        return h.id !== id;
      });
      localStorage.setItem(STORAGE_KEY, JSON.stringify(arr));
      return true;
  ...
  window.CloudData = {
    ...
    syncScales: function (scales) {
      ...
      const openid = CloudAPI.getOpenId();
      if (!openid) {
    ...
    loadCloudScales: function () {
      ...
      const lnmpPromise = this._loadFromLNMP();
      if (!window.CloudAPI || !CloudAPI.isAvailable()) {
        return lnmpPromise;
      }
      // LNMP 成功则直接返回，失败再尝试云托管
      return lnmpPromise.then(function (lnmpData) {
        if (lnmpData && lnmpData.length > 0) {
          return lnmpData;
        }
    ...
    _loadFromLNMP: function () {
      const lnmpBase = 'https://www.soarto.com.cn';
