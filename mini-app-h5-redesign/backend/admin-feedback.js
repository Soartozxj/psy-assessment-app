      ...
      tbody.innerHTML = pageData
        .map(function (f) {
          ...
          return (
            ...
            dateStr +
            '</div></td>' +
            '<td><span class="fb-admin-action" onclick="fbAdminShowDetail(' +
            f.id +
            ')">📋 详情</span><span class="fb-admin-action danger" onclick="fbAdminDelete(' +
            f.id +
            ')">🗑️ 删除</span></td>' +
            '</tr>'
          );
...
function fbAdminShowDetail(id) {
  ...
  const html =
    ...
    '</div>' +
    assessHtml +
    '<div class="fb-detail-actions"><button class="btn btn-danger" style="padding:6px 16px;font-size:12px" onclick="fbAdminDelete(' +
    f.id +
  ...
  el.innerHTML = tags
    .map(function (t, i) {
      ...
      return (
        ...
        maxLen +
        '</span>' +
        '<span class="fb-tag-del" onclick="fbTagDelete(' +
        i +
        ')" title="删除">✕</span>' +
        '</div>'
      );
