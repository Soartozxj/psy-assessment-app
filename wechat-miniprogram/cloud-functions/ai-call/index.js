...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // temperature 范围限制
    const effectiveTemp = Math.max(0, Math.min(2, temperature !== undefined ? temperature : (aiConfig.temperature || 0.7)))
    // maxTokens 范围限制
    ...
    if (effectiveProvider === 'dashscope') {
      ...
      result = data.choices && data.choices[0] && data.choices[0].message
