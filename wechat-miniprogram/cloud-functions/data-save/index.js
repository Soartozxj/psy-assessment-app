...
const ALLOWED_RECORD_FIELDS = [
  'id', 'scaleId', 'scaleName', 'emoji', 'color',
  'score', 'displayTotalScore', 'levelName', 'level',
  'dims', 'answers', 'duration', 'completedAt',
  'aiDiagText', 'npcConfig'
]
  ...
  try {
    ...
    if (filteredRecord.scaleId) {
      ...
      if (scaleDoc.data.length > 0) {
        await db.collection('scales').doc(scaleDoc.data[0]._id).update({
