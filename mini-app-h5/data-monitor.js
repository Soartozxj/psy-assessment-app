            ...
            try {
                ...
                if (backendData) {
                    try {
                        ...
                        status.backendScales = Array.isArray(parsed) ? parsed.map(s => ({id: s.id, name: s.name, status: s.status})) : [];
                    } catch(e) {
                ...
                if (frontendData) {
                    try {
                        ...
                        status.frontendScales = Array.isArray(parsed) ? parsed.map(s => ({id: s.id, name: s.name, status: s.status})) : [];
                    } catch(e) {
                ...
                if (sharedData) {
                    try {
                        ...
                        status.sharedScales = Array.isArray(parsed) ? parsed.map(s => ({id: s.id, name: s.name, status: s.status})) : [];
                    } catch(e) {
                ...
                if (status.backendCount !== status.frontendCount && backendData) {
                    status.consistencyIssues.push(`数据数量不一致：后端 ${status.backendCount} 个 vs 前端 ${status.frontendCount} 个`);
                    status.recommendations.push('点击"同步到前端"按钮更新前端数据');
                }
                
                if (status.sharedCount !== status.backendCount && sharedData) {
                    status.consistencyIssues.push(`共享数据与后端不一致：共享 ${status.sharedCount} 个 vs 后端 ${status.backendCount} 个`);
                ...
                if (activeFrontendScales.length !== activeBackendScales.length) {
                    status.consistencyIssues.push(`活跃量表数量不一致：前端 ${activeFrontendScales.length} 个 vs 后端 ${activeBackendScales.length} 个`);
