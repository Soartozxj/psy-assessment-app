...
(function () {
  ...
  window.AdminAPI = {
    ...
    saveSystemPrompt: async function (scaleId, promptConfig) {
      try {
        const scales = await this.getScales();
        const idx = scales.findIndex(function (s) {
          return String(s.id) === String(scaleId);
        });
    ...
    batchSaveSystemPrompt: async function (updates) {
      ...
      for (let i = 0; i < updates.length; i++) {
        ...
        const ok = await this.saveSystemPrompt(item.scaleId, item.promptConfig);
        ok ? success++ : failed++;
      }
    ...
    importScales: async function (jsonStr, merge) {
      try {
        ...
        let currentScales = SharedData.getAllScales();

        if (merge) {
          // 合并模式：按 ID 合并或追加
          const existingIds = currentScales.map(function (s) {
            return String(s.id);
          });
          newScales.forEach(function (ns) {
            const idx = existingIds.indexOf(String(ns.id));
            if (idx >= 0) {
    ...
    getTestSuite: function () {
      try {
        const stored = localStorage.getItem(STORAGE_KEYS.TEST_SUITE);
