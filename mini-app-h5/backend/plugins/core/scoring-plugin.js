/**
 * scoring-plugin.js - 计分规则插件（完整版）
 *
 * 大白话解释：
 * - 负责任务计分规则的管理
 * - 继承自PluginBase，获得标准接口
 * - 使用Adapter适配H5和小程序双端
 * - 包含维度配置、指标配置、解释规则、筛查规则、模拟测试等功能
 *
 * @version 2.0.0
 * @date 2026-06-02
 */
...
class ScoringPlugin extends PluginBase {
  ...
  async onInit() {
    ...
    try {
      ...
      console.log('✅ 计分规则插件初始化完成');
    } catch (error) {
      console.error('❌ 计分规则插件初始化失败:', error);
      throw error;
    }
  }
  ...
  async onExecute(params = {}) {
    ...
    switch (action) {
      ...
      case 'get':
        return await this._getScale(params.id);
      case 'select':
        return await this._selectScale(params.scaleId);
      case 'render-list':
        return await this._renderScoringScaleList();
      case 'load-form':
        return await this._loadScoringFormInline(params.scaleId);
      case 'save':
      ...
      case 'switch-panel':
        return await this._switchScPanel(params.btn, params.idx);
      default:
        throw new Error(`未知的计分规则操作: ${action}`);
    }
  }
  ...
  async _loadScales() {
    ...
    // 从全局变量 scales 获取 data（适配原有逻辑）
    if (typeof scales !== 'undefined') {
      this._scales = scales;
    } else {
      // 如果全局变量不存在，从 localStorage 加载
      const scalesData = localStorage.getItem('scalesData');
      this._scales = scalesData ? JSON.parse(scalesData) : [];
    }
   ...
   */
  _getScalesList() {
    return this._scales.map((s) => ({
      id: s.id,
      name: s.name,
      code: s.code,
      questionCount: (s.questions || []).length,
      hasScoring: !!(s.scoring && (s.scoring.dimensions || []).length > 0)
    }));
  }
   ...
   */
  _getScale(id) {
    const scale = this._scales.find((s) => s.id === id);
    if (!scale) {
      throw new Error(`量表不存在: ${id}`);
    }
    return scale;
  }
   ...
   * 选择量表
   */
  async _selectScale(scaleId) {
    this._currentScaleId = scaleId;
    ...
    // 加载表单
    await this._loadScoringFormInline(scaleId);
  ...
  async _renderScoringScaleList() {
    ...
    const filtered = this._scales.filter((s) => s.name.toLowerCase().includes(keyword));

    container.innerHTML = filtered
      .map((s) => {
        const qCount = (s.questions || []).length;
        const hasScoring = !!(s.scoring && (s.scoring.dimensions || []).length > 0);
        return `<div class="sc-scale-item" data-action="select-scoring-scale" data-scale-id="${s.id}" style="padding:10px 12px;margin-bottom:4px;border-radius:6px;cursor:pointer;border:1px solid transparent;transition:all 0.15s;">
  <div style="font-size:13px;font-weight:500;color:var(--text);margin-bottom:2px;">${this._escHtml(s.name)}</div>
   ...
   * 加载计分表单（内嵌模式）
   */
  async _loadScoringFormInline(scaleId) {
    if (typeof document === 'undefined') {
      return;
    }
    ...
    const scale = this._scales.find((s) => s.id === scaleId);
    if (!scale) {
      return;
    }

    // 构建表单 HTML
    container.innerHTML = this._buildScoringFormHTML(scaleId);
    ...
    // 渲染维度
    dims.forEach((d, i) => this._renderDimension(d, i));

    // 渲染指标
    metrics.forEach((m, i) => this._renderMetric(m, i));

    // 渲染解释规则
    interp.forEach((r, i) => this._renderInterpRule(r, i, metrics, dims));

    // 渲染筛查规则
    this._renderScreening(screening, metrics, dims);
  }
...
<div class="sc-panel" id="sc-panel-0">
  ${unusedDims.length > 0 ? `<div style="padding:10px 14px;background:#FFF8E1;border:1px solid #F5A623;border-radius:6px;font-size:12px;color:#8B6914;margin-bottom:16px;">💡 题目中已标记但未配置的维度：${unusedDims.map((d) => '<strong>' + this._escHtml(d) + '</strong>').join('、')}，建议添加对应维度规则。</div>` : ''}
...
`;
   ...
   */
  async _addDimension() {
    const scale = this._getSelectedScale();
    if (!scale) {
      return;
    }
...
<div style="display:flex;gap:6px;margin-bottom:6px;align-items:center;flex-wrap:wrap;">
  ...
  <input type="text" placeholder="等级标签" value="${this._escHtml(r.label || '')}" data-role="interp-label" style="width:80px;padding:6px;border:1px solid var(--border);border-radius:4px;font-size:12px;">
  ...
  <input type="text" placeholder="解释文本" value="${this._escHtml(r.text || '')}" data-role="interp-text" style="flex:1;min-width:120px;padding:6px;border:1px solid var(--border);border-radius:4px;font-size:12px;">
...
<div class="sc-dim-card" id="${id}" style="border:1px solid var(--border);border-radius:8px;padding:14px;margin-bottom:12px;background:#fff;">
  <div style="display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none'">
    <span style="font-size:16px;font-weight:600;color:var(--primary);">${this._escHtml(dim.label || '新维度')}</span>
  ...
  <div style="display:${isCollapsed ? 'none' : 'block'};margin-top:12px;">
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px;">
      <div>
        ...
        <input type="text" value="${this._escHtml(dim.key || '')}" data-role="dim-key" placeholder="如 somatization" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
      </div>
      <div>
        <label style="font-size:12px;color:var(--text-sec);display:block;margin-bottom:4px;">维度名称（中文）</label>
        <input type="text" value="${this._escHtml(dim.label || '')}" data-role="dim-label" placeholder="如 躯体化" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
      </div>
      <div>
...
<div id="${id}" style="border:1px solid var(--border);border-radius:8px;padding:14px;margin-bottom:12px;background:#fff;">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
    <span style="font-size:14px;font-weight:600;">${this._escHtml(metric.label || '新指标')}</span>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px;">
    <div>
      ...
      <input type="text" value="${this._escHtml(metric.key || '')}" data-role="metric-key" placeholder="如 totalScore" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
    </div>
    <div>
      <label style="font-size:12px;color:var(--text-sec);display:block;margin-bottom:4px;">指标名称</label>
      <input type="text" value="${this._escHtml(metric.label || '')}" data-role="metric-label" placeholder="如 总分" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
    </div>
    <div>
      <label style="font-size:12px;color:var(--text-sec);display:block;margin-bottom:4px;">计算公式</label>
...
`;
  ...
  _renderInterpRule(rule, index, metrics, dimensions) {
    ...
    const metricOptions = (metrics || [])
      .map(
        (m) =>
          `<option value="${this._escHtml(m.key)}" ${rule.metric === m.key ? 'selected' : ''}>${this._escHtml(m.label || m.key)}</option>`
      )
      .join('');
    const dimOptions = (dimensions || [])
      .map(
        (d) =>
          `<option value="${this._escHtml(d.key)}" ${rule.metric === d.key ? 'selected' : ''}>${this._escHtml(d.label || d.key)}</option>`
      )
      .join('');
...
<div style="display:flex;gap:8px;margin-bottom:8px;align-items:center;flex-wrap:wrap;" class="sc-interp-row">
  ...
  <input type="text" value="${this._escHtml(rule.label || '')}" data-role="interp-label" placeholder="标签" style="width:80px;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
  ...
  <input type="text" value="${this._escHtml(rule.text || '')}" data-role="interp-text" placeholder="解释文本" style="flex:1;min-width:120px;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
...
<div class="sc-screen-cond" style="display:flex;gap:8px;margin-bottom:8px;align-items:center;">
  <select data-role="scr-metric" style="padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
    ${allOpts.map((o) => `<option value="${o.key}" ${c.metric === o.key ? 'selected' : ''}>${this._escHtml(o.label)}</option>`).join('')}
  ...
  <input type="text" value="${this._escHtml(c.label || '')}" data-role="scr-label" placeholder="标签说明" style="width:100px;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
  <button data-action="remove-parent" style="padding:6px 10px;background:var(--danger);color:#fff;border:none;border-radius:6px;cursor:pointer;">✕</button>
</div>
`
      )
...
`;
  ...
  async _addScreenCond() {
    ...
    const dimOptions = dims
      .map((d) => `<option value="${d.key}">${this._escHtml(d.label || d.key)}（维度）</option>`)
      .join('');
    const metOptions = metrics
      .map((m) => `<option value="${m.key}">${this._escHtml(m.label || m.key)}（指标）</option>`)
      .join('');
...
`;
  ...
  async _runScoringTest() {
    ...
    // 临时收集当前表单数据
    const tempScoring = this._collectScoringData();
    ...
    // 使用 ScoringEngine 计算（假设已全局加载）
    let result = {};
    if (typeof ScoringEngine !== 'undefined') {
      result = ScoringEngine.calculate(scaleWithScoring, answers);
    } else {
      // 模拟计算结果
      result = {
  ...
  <div style="display:grid;gap:8px;">
    ${result.dimensions
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg);border-radius:6px;">
        <span style="font-size:13px;font-weight:500;min-width:80px;">${this._escHtml(d.label)}</span>
...
<div style="margin-top:12px;padding:12px;background:${result.screening.result === (scaleWithScoring.scoring.screening || {}).positiveLabel ? '#FFF2F0' : '#F0FFF0'};border:1px solid ${result.screening.result === (scaleWithScoring.scoring.screening || {}).positiveLabel ? '#FFB3B3' : '#B3FFB3'};border-radius:8px;font-size:13px;">
  ...
  ${result.screening.triggeredRules && result.screening.triggeredRules.length > 0 ? `（触发：${result.screening.triggeredRules.map((r) => r.metric + r.op + r.value).join(', ')}）` : ''}
</div>`
    : ''
}
...
`;
  ...
  _collectScoringData() {
    ...
    document.querySelectorAll('.sc-dim-card').forEach((card) => {
      ...
      const condition = this._parseConditionStr(condStr);
  ...
  async _saveScoringConfig() {
    const scaleId = this._currentScaleId;
    if (!scaleId) {
      if (typeof UIUtils !== 'undefined') {
        UIUtils.showToast('请先选择一个量表', 'warning');
      }
      return;
    }
    ...
    const data = this._collectScoringData();
    scale.scoring = data;

    const idx = this._scales.findIndex((s) => s.id === scale.id);
    if (idx >= 0) {
      this._scales[idx] = scale;
    }

    // 保存到 localStorage（适配原有逻辑）
    if (typeof SharedData !== 'undefined') {
      SharedData.saveScalesData(this._scales);
    ...
    if (typeof UIUtils !== 'undefined') {
      UIUtils.showToast('计分规则已保存！', 'success');
    }
  ...
  async _resetScoring() {
    ...
    if (typeof UIUtils !== 'undefined') {
      const confirmed = await UIUtils.showConfirm('确定要重置该量表的计分规则吗？', '确认重置');
      if (!confirmed) {
        return;
      }
    } else {
    ...
    if (typeof UIUtils !== 'undefined') {
      UIUtils.showToast('计分规则已重置', 'success');
    }
   ...
   */
  _getSelectedScale() {
    if (!this._currentScaleId) {
      return null;
    }
    return this._scales.find((s) => s.id === this._currentScaleId) || null;
  }
...
// 导出到全局
window.ScoringPlugin = ScoringPlugin;

console.log('✅ scoring-plugin.js v2.0.0 加载完成');
