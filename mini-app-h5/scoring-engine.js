...
var ScoringEngine = (() => {
  ...
  function applyFormula(formula, scores, condition) {
    ...
    switch (formula) {
      ...
      case 'COUNT_IF': {
        ...
        if (!norm) return scores.length; // 无条件 = 统计总数
  ...
  function evaluateScreening(screening, metricValues) {
    ...
    const results = screening.conditions.map(cond => {
      ...
      if (actual === undefined) {
        ...
        if (matched) actual = typeof metricValues[matched] === 'object' ? metricValues[matched].score : metricValues[matched];
      }
      if (actual === undefined) {
        // 尝试在维度结果中查找
      ...
      // 如果值是对象（维度结果），取 score
      if (typeof actual === 'object' && actual.score !== undefined) actual = actual.score;
  ...
  function inferMaxFromInterpretation(interpretation) {
    ...
    const hasOpenEnded = interpretation.some(r => r.min !== undefined && r.min !== null && (r.max === undefined || r.max === null));
    if (hasOpenEnded) return null;
  ...
  function calcMaxScores(scoring, questions, actualMetrics) {
    ...
    if (scoring.metrics && scoring.metrics.length > 0) {
      ...
      let tsMetric = scoring.metrics.find(m =>
        (m.key || m.name) === 'totalScore' || (m.key || m.name) === 'total_score' ||
        (m.key || m.name) === '总分' || (m.key || m.name) === 'metric_total_score'
      );
      ...
      if (!tsMetric) {
        tsMetric = scoring.metrics.find(m => {
          ...
          return l.includes('总分') || l.includes('total') ||
                 k.includes('total') || k.includes('总分');
        });
      }
      if (!tsMetric) {
        // 兜底：顶层 interpretation 中有对应规则的 metric，且为汇总型公式
        const interpMetricKeys = new Set(
          (scoring.interpretation || []).filter(r => r.metric).map(r => r.metric)
        );
      ...
      if (tsMetric) {
        ...
        if (tsMetric.maxScore !== undefined && tsMetric.maxScore > 0) {
          ...
          const tsRules = (scoring.interpretation || []).filter(r =>
            r.metric === tsKey
          );
          if (tsRules.length > 0) {
  ...
  function calcMinScores(scoring, questions) {
    ...
    // 2. 总分最低分
    if (scoring.metrics && scoring.metrics.length > 0) {
      const tsMetric = scoring.metrics.find(m =>
      ...
      if (tsMetric) {
        ...
        const tsKey = tsMetric.key || tsMetric.name;
        const tsRules = (scoring.interpretation || []).filter(r =>
          !r.metric || r.metric === tsKey || r.metric === 'totalScore' || r.metric === 'total_score' || r.metric === '总分'
        );
        if (tsRules.length > 0) {
  ...
  function normalizeLevel(label) {
    ...
    if (/^(medium|moderate|middle|moderate)$/i.test(lower.trim())) return 'moderate';
  ...
  function normalizeRangeBounds(min, max) {
    let nMin = min !== undefined ? Number(min) : undefined;
    let nMax = max !== undefined ? Number(max) : undefined;
  ...
  function normalizeAiJson(aiJson) {
        ...
        if (dim.interpretation && dim.interpretation.length > 0) {
          dimEntry.interpretation = dim.interpretation.map(r => {
            ...
            if (r.condition && min === undefined && max === undefined) {
              ...
              if (parsed) {
                if (parsed.operator === '>' || parsed.operator === '>=') min = parsed.value;
                else if (parsed.operator === '<' || parsed.operator === '<=') max = parsed.value;
              }
            }
    ...
    if (aiJson.metrics) {
      for (const m of aiJson.metrics) {
        ...
        if (key === 'total_score' || key === 'Total_Score') key = 'totalScore';
        const entry = {
          key,
          label: mLabel,
    ...
    if (aiJson.interpretation) {
      for (const rule of aiJson.interpretation) {
        ...
        if (metricKey === 'total_score' || metricKey === 'Total_Score') metricKey = 'totalScore';
        if (metricKey) {
          ...
          if (!metMatch) {
            ...
            if (metByLabel) metricKey = metByLabel.key;
          }
          ...
          if (!metMatch && !scoring.metrics.find(m => m.key === metricKey)) {
            ...
            if (dimMatch) metricKey = dimMatch.key;
          }
        }
  ...
  function pinyinToKey(label, prefix) {
    if (!label) return (prefix || 'metric_') + Date.now();
    const map = {
      '总分': 'total_score', '总均分': 'average_score', '标准分': 'standard_score',
      '原始总分': 'raw_total', '粗分': 'raw_score',
      '阳性项目数': 'positive_count', '阴性项目数': 'negative_count',
      '阳性症状均分': 'positive_average', '阳性症状痛苦水平': 'pdi',
      '躯体化': 'somatization', '强迫症状': 'obsessive_compulsive',
      '强迫': 'obsessive', '人际关系敏感': 'interpersonal_sensitivity',
      '人际敏感': 'interpersonal', '抑郁': 'depression',
      '焦虑': 'anxiety', '敌对': 'hostility', '恐怖': 'phobia',
      '偏执': 'paranoid', '精神病性': 'psychoticism',
      '其他': 'other', '外向性': 'extraversion', '宜人性': 'agreeableness',
      '尽责性': 'conscientiousness', '神经质': 'neuroticism', '开放性': 'openness'
    };
